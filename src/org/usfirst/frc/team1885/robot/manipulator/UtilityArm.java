...
public class UtilityArm implements Module {
    ...
    public void changeValues() {
        ...
        jointBAngle = 180 - Math.toDegrees(Math.asin(Math.sin(Math.toRadians(getAngleB())) + getJoystickYConversion()));
        jointAAngle = Math.toDegrees(Math.acos(Math.cos(Math.toRadians(getAngleA())) - 
                (Math.cos(Math.toRadians(jointBAngle)) - Math.cos(Math.toRadians(getAngleB())))));
        
        //change in X
        jointAAngle = Math.toDegrees(Math.acos(Math.cos(Math.toRadians(jointAAngle)) + getJoystickXConversion()));
        jointBAngle = 180 - Math.toDegrees(Math.asin(Math.sin(Math.toRadians(jointBAngle)) -
