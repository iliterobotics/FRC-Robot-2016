 ...
 * @version 2/25/2016
 * 
 *          AuxArm is the code for the side arm of the 2016 Stronghold Robot
...
public class AuxArm implements Module {
    ...
    protected AuxArm() {
        ...
        RESET_A_POSITION = SensorInputControlSRX.getInstance().INITIAL_POT_A_POSITION;
        RESET_B_POSITION = SensorInputControlSRX.getInstance().INITIAL_POT_B_POSITION;
    ...
    public void update() {
        ...
        if (Math.abs(driverInputControl.getControllerTwist()) > DEAD_ZONE_X
                || Math.abs(driverInputControl.getControllerThrottle()) > DEAD_ZONE_Y) {
            if (Math.abs(
